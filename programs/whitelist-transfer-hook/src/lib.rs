...
pub mod whitelist_transfer_hook {
    ...
    pub fn initialize_whitelist(ctx: Context<InitializeWhitelist>, address: Pubkey) -> Result<()> {
        ctx.accounts.initialize_whitelist(address, ctx.bumps)
    }

    pub fn remove_whitelist(ctx: Context<RemoveWhitelist>, _address: Pubkey) -> Result<()> {
